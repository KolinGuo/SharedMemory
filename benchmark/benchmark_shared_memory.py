...
if __name__ == "__main__":
    ...
    for object_type_idx in range(len(SharedObject._object_types) - 1):
        mean_ns, std_ns = benchmark_object_2_proc_fetch_assign(
            object_type_idx, n_iters=n_iters
        )
    ...
    results_table.field_names = [
        "Object Type / Duration for 1 obj (mean \xb1 std)"
    ] + list(results[0].keys())
    ...
    print(
        f"\nComparing with {str(prev_npz.name)}\n\tshow if mean has increased"
        f" ({up_arrow_str}) or decreased ({down_arrow_str}) with (mean - prev_mean) and"
    ...
    N_iters = np.array(
        [[1000] * 7 + [100, 100]]
        * 8  # NoneType, bool, int, float, Pose, str, bytes, ndarray
        + [
            [500] + [1000] * 6 + [100, 100],  # ndarray (720, 1280, 3)
            [10, 20, 50, 50, 50, 50, 50, 10, 10],
        ]  # ndarray (10000, 10000)
    )
    ...
    for i, (idx, res_dict) in enumerate(results.items()):
        ...
        for j, res_key in enumerate(res_dict):
            ...
            conf_level = compute_conf_level(
                mean_ns, std_ns, prev_mean_ns, prev_std_ns, N_iters[i, j]
            )
